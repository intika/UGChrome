...
class NativeBackendKWalletTestBase :
 ...
 protected:
  NativeBackendKWalletTestBase() {
    old_form_google_.origin = GURL("http://www.9oo91e.qjz9zk/");
    old_form_google_.action = GURL("http://www.9oo91e.qjz9zk/login");
    ...
    form_google_.icon_url = GURL("http://www.9oo91e.qjz9zk/icon");
...
TEST_P(NativeBackendKWalletTest, BasicUpdateLogin) {
  ...
  new_form_google.action = GURL("http://www.9oo91e.qjz9zk/different/login");
...
TEST_P(NativeBackendKWalletTest, ReadDuplicateForms) {
  ...
  form_google_.origin =
      GURL(std::string("http://www.9oo91e.qjz9zk/") + unique_string);
  backend.GetBackgroundTaskRunner()->PostTask(
      FROM_HERE,
      base::BindOnce(base::IgnoreResult(&NativeBackendKWalletStub::AddLogin),
                     base::Unretained(&backend), form_google_));
  form_google_.origin =
      GURL(std::string("http://www.9oo91e.qjz9zk/") + unique_string_replacement);
  backend.GetBackgroundTaskRunner()->PostTask(
      FROM_HERE,
      base::BindOnce(base::IgnoreResult(&NativeBackendKWalletStub::AddLogin),
                     base::Unretained(&backend), form_google_));
